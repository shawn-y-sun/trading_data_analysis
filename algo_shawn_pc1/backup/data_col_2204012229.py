...
# CHANGE before run
algo_file = r'Team12_ALGO2_2_Strat1_v1.0'
trader = 'Shawn-PC1'
file_path = "algo2_data_shawn_pc1.xlsx"
s = requests.Session()
s.headers.update({'X-API-key': 'YI1W7QRM'})
...
def get_ticker_traders_info(ticker, count_dict=traders_info_dict):
...
def get_traders_info(wait=0.5):
    
    tick, status = get_tick()
    while status == 'ACTIVE':
    
        for ticker in ticker_lst:
            get_ticker_traders_info(ticker)
            tick, status = get_tick()
...
def collect_input():
    ...
    traders_input = parse_traders_info()
...
if __name__ == '__main__':
    while True:
        tick, status = get_tick()
        if status == 'ACTIVE':
            globals()['traders_info_dict'] = {ticker: {} for ticker in ticker_lst}
            write_data()
            sleep(2)
            tick, status = get_tick()
